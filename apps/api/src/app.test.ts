...
const services: PlatformServices = {
  createBinding: async () => ({ contractVersion: "1", bindingId: "00000000-0000-0000-0000-000000000001", identityId: "00000000-0000-0000-0000-000000000002", provider: "qq", externalUserId: "user-1", playerHandle: "Player#1234" }),
...
describe("API", () => {
  ...
  it("validates and authenticates binding creation", async () => {
    const response = await app.request("http://localhost/v1/qq/bindings", { method: "POST", headers: { "idempotency-key": "binding-1", "content-type": "application/json" }, body: JSON.stringify({ contractVersion: "1", provider: "qq", externalUserId: "user-1", playerHandle: "Player#1234" }) }, env);
    expect(response.status).toBe(201);
    expect((await response.json() as { provider: string }).provider).toBe("qq");
  });

  it("rejects requests without an idempotency key", async () => {
    const response = await app.request("http://localhost/v1/qq/bindings", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ contractVersion: "1", provider: "qq", externalUserId: "user-1", playerHandle: "Player#1234" }) }, env);
