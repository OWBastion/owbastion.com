...
<template><main class="events-page page-shell"><section class="page-intro"><h1 class="page-title">随机事件</h1><p class="body-copy">查看已实装事件、历史记录与开放挑战。</p></section><section class="events-panel surface-card"><p v-if="loading" class="state">读取中…</p><UAlert v-else-if="error" color="error" variant="subtle" title="无法读取事件" :description="error" /><EventDirectory v-else :events="events" /></section></main></template>
