...
const title = computed(() => entry.value ? `${entry.value.title} · 版本更新 · 躲避堡垒 3` : "版本更新 · 躲避堡垒 3");
...
<template>
  <main class="editorial-detail-page page-shell--narrow">
    <NuxtLink to="/changelog" class="editorial-back-link pressable">返回版本更新</NuxtLink>
    <div v-if="status === 'pending'" class="editorial-detail-state surface-card" role="status">正在读取版本更新…</div>
    <UAlert v-else-if="error" color="error" variant="subtle" title="无法读取版本更新" description="内容暂时不可用，请稍后重试。" />
    <UAlert v-else-if="!entry" color="neutral" variant="subtle" title="找不到这条版本更新" description="该地址对应的内容不存在或已移除。">
