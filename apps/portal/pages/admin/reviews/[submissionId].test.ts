...
describe("admin review detail page", () => {
  ...
  it("lets maintainers select an automatic-match challenge", async () => {
    ...
    await flushPromises();
    expect(adminApi).toHaveBeenCalledWith("/v1/submissions/submission-1/challenge", expect.objectContaining({ method: "POST", body: { contractVersion: "1", challengeId: "map.paraiso.hell", mapId: "map.paraiso" } }));
  });
