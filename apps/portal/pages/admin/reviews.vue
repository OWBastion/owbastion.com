...
async function open(submission: AdminSubmission) {
  reviewError.value = "";
  clearEvidenceImage();
  try {
    const detail = await api<AdminSubmission>(`/v1/submissions/${submission.submissionId}`);
    ...
    selected.value = { ...detail, evidenceUrl: evidenceImageUrl.value };
  } catch (error) { reviewError.value = portalErrorDetails(error, "无法读取待核对截图，请稍后重试。").description; }
}
...
function clearEvidenceImage() { if (evidenceImageUrl.value?.startsWith("blob:")) URL.revokeObjectURL(evidenceImageUrl.value); evidenceImageUrl.value = null; }
function close() { clearEvidenceImage(); selected.value = null; }
onMounted(() => { void load(); });
