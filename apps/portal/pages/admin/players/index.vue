  ...
  <AdminWorkspace title="玩家管理" :count="loading ? '读取中…' : `${total} 条`">
    ...
    <section aria-label="玩家帐号">
      <AdminDataTable v-model:column-filters="statusColumnFilters" v-model:global-filter="query" v-model:sorting="playerSorting" :sorting-options="playerSortingOptions" :default-sorting="defaultPlayerSorting" :data="players" :columns="columns" :mobile-columns="[{ id: 'playerName', priority: 'primary', order: 0 }, { id: 'status', priority: 'primary', order: 1 }, { id: 'bindingCount', priority: 'detail', order: 2 }, { id: 'updatedAt', priority: 'detail', order: 3 }]" row-key="playerAccountId" :mobile-row-link="(row) => `/admin/players/${row.playerAccountId}`" :loading="loading" empty="暂无匹配玩家。" table-key="players" manual-filtering :reset-scroll-key="`${page}-${JSON.stringify(statusColumnFilters)}`" class="admin-table">
        ...
        <template #mobile-primary><UInput v-model="query" class="w-full" size="md" aria-label="搜索玩家" placeholder="搜索战网 ID 或 QQ 标识" /></template>
        <template #mobile-secondary><USelect v-model="status" size="md" aria-label="筛选玩家状态" :items="[{ label: '全部状态', value: 'all' }, { label: '正常', value: 'active' }, { label: '已封禁', value: 'banned' } ]" /></template>
