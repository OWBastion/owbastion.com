...
<template>
  <AdminWorkspace :title="player ? `${player.playerName}#${player.playerId}` : '玩家详情'">
    ...
    <AdminResponsiveDialog :open="pendingAction !== null" :title="actionTitle" :description="actionDescription" size="sm" :dismissible="!actionLoading" @update:open="(open) => { if (!open) closeAction(); }">
      ...
      <template #footer><UButton :label="actionTitle" :color="pendingAction?.type === 'set-status' && pendingAction.status === 'banned' ? 'error' : 'primary'" :variant="pendingAction?.type === 'set-status' && pendingAction.status === 'banned' ? 'soft' : 'solid'" type="submit" form="player-action" :loading="actionLoading" /><UButton label="取消" color="neutral" variant="outline" :disabled="actionLoading" @click="closeAction()" /></template>
