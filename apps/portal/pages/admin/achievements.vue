          ...
          <AdminResponsiveDialog v-model:open="editorOpen" title="编辑规则" :description="editingItem ? itemName(editingItem) : undefined" size="lg">
            <template #body>
              <form v-if="editingItem" id="achievement-editor" class="editor" @submit.prevent="saveEditingItem(editingItem)">
                ...
                <template v-if="isChallengeTitle(editingItem)"><UFormField class="editor-field" label="挑战范围"><USelect class="editor-control" :model-value="editingItem.scope ?? 'global'" :disabled="isSaving(editingItem)" :items="[{ label: '通用挑战', value: 'global' }, { label: '地图挑战', value: 'map' }]" @update:model-value="setScope($event as 'global' | 'map')" /></UFormField><UFormField v-if="editingItem.scope === 'map'" class="editor-field editor-field--wide" label="指定地图"><USelect class="editor-control" :model-value="editingItem.mapIds ?? []" multiple :items="[{ label: '全部有效地图', value: '' }, ...maps.map((map) => ({ label: map.mapName, value: map.mapId }))]" :disabled="isSaving(editingItem)" @update:model-value="setMapIds(($event as string[]).filter(Boolean))" /></UFormField></template>
