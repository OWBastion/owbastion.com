...
<template>
  <AdminWorkspace title="地图管理" :count="loading ? '读取中…' : `${maps.length} 张`">
    ...
    <section aria-label="地图目录">
      <AdminDataTable v-model:global-filter="globalFilter" v-model:sorting="mapSorting" :sorting-options="mapSortingOptions" :default-sorting="defaultMapSorting" :data="mapRows" :columns="columns" :mobile-columns="[{ id: 'mapName', priority: 'primary', order: 0 }, { id: 'difficultyRating', priority: 'primary', order: 1 }, { id: 'challengeCount', priority: 'primary', order: 2 }, { id: 'mechanics', priority: 'detail', order: 3 }, { id: 'gameVersion', priority: 'detail', order: 4 }]" row-key="mapId" :loading="loading" empty="暂无地图记录。" table-key="maps" class="admin-table">
        ...
        <template #mobile-primary><UInput v-model="query" class="w-full" size="md" aria-label="搜索地图" placeholder="搜索地图名称或 ID" /></template>
        <template #mobile-secondary><USelect v-model="ratingFilter" size="md" aria-label="筛选地图评级" :items="[{ label: '全部评级', value: 'all' }, ...ratings.map((rating) => ({ label: rating, value: rating }))]" /></template>
        ...
        <template #actions-cell="{ row }"><div class="table-actions"><UButton label="查看" size="sm" color="neutral" variant="outline" @click="openMap(maps.find((map) => map.mapId === row.original.mapId)!)" /></div></template>
      </AdminDataTable>
    </section>
    ...
    <AdminResponsiveDialog v-model:open="panelOpen" :title="selectedMap ? `${selectedMap.mapName} · 地图属性` : ''" size="md">
      <template #body>
        <section v-if="selectedMap" class="map-detail">
          ...
          <form id="map-metadata-editor" class="editor" @submit.prevent="saveMetadata">
          ...
          <section class="challenge-section" aria-labelledby="map-challenges-title"><div class="section-heading"><div><p class="eyebrow">挑战目录</p><h3 id="map-challenges-title">挑战难度</h3></div><span>{{ selectedChallenges.length }} 项</span></div><AdminDataTable v-model:sorting="challengeSorting" :sorting-options="challengeSortingOptions" :default-sorting="defaultChallengeSorting" :data="selectedChallenges" :columns="challengeColumns" :loading="loading" empty="暂无挑战记录。" row-key="challengeId" :table-key="`map-challenges-${selectedMap.mapId}`" class="admin-table nested-table"><template #name-cell="{ row }"><strong>{{ row.original.name }}</strong><small class="table-meta">{{ mapVariantLabel(row.original.mapVariant) }}</small></template><template #difficulty-cell="{ row }"><span>{{ row.original.difficulty ?? '地图通关' }}</span></template><template #status-cell="{ row }"><StatusBadge :label="statusText(row.original.status)" :tone="statusTone(row.original.status)" /></template><template #actions-cell="{ row }"><div class="table-actions"><NuxtLink class="table-meta" :to="{ path: '/admin/achievements', query: { section: 'map', mapId: row.original.mapId, challengeId: row.original.challengeId, ...(row.original.mapTitleRule?.dynamic ? { ruleId: row.original.mapTitleRule.ruleId } : {}) } }">{{ row.original.mapTitleRule?.dynamic ? '查看来源规则' : '在成就与称号中管理' }}</NuxtLink></div></template></AdminDataTable></section>
        </section>
      </template>
