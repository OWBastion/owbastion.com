...
async function save() { saving.value = true; error.value = ""; const body = { contractVersion: "1" as const, name: form.name, category: form.category, rarity: form.rarity, description: form.description, durationSeconds: number(form.durationSeconds), cooldownSeconds: number(form.cooldownSeconds), weight: number(form.weight), gameVersion: form.gameVersion, effectTags: form.effectTags.map((value) => value.trim()).filter(Boolean), releaseStatus: form.releaseStatus, challengeLinks: form.links }; try { const saved = selectedEvent.value ? await api<RandomEvent>(`/v1/events/${encodeURIComponent(selectedEvent.value.eventId)}`, { method: "PUT", headers: { "Idempotency-Key": crypto.randomUUID() }, body }) : await api<RandomEvent>("/v1/events", { method: "POST", headers: { "Idempotency-Key": crypto.randomUUID() }, body }); events.value = selectedEvent.value ? events.value.map((event) => event.eventId === saved.eventId ? saved : event) : [saved, ...events.value]; selectedEvent.value = saved; editorOpen.value = false; toast.add({ title: "事件已保存", color: "success" }); } catch (cause) { error.value = portalErrorDetails(cause, "无法保存事件。").description; } finally { saving.value = false; } }
function requestArchive() { archiveOpen.value = true; }
async function archive() { if (!selectedEvent.value) return; saving.value = true; try { const eventId = selectedEvent.value.eventId; await api(`/v1/events/${encodeURIComponent(eventId)}`, { method: "DELETE", headers: { "Idempotency-Key": crypto.randomUUID() } }); events.value = events.value.filter((event) => event.eventId !== eventId); archiveOpen.value = false; editorOpen.value = false; selectedEvent.value = null; toast.add({ title: "事件已归档", color: "success" }); } catch (cause) { error.value = portalErrorDetails(cause, "无法归档事件。").description; } finally { saving.value = false; } }
  ...
  <AdminWorkspace title="事件管理" :count="loading ? '读取中…' : `${events.length} 条`">
    ...
    <section aria-label="事件目录">
      <AdminDataTable v-model:global-filter="query" v-model:sorting="sorting" v-model:grouping="grouping" v-model:column-pinning="columnPinning" :data="events" :columns="eventColumns" :loading="loading" :sorting-options="eventSortingOptions" :grouping-options="eventGroupingOptions" :default-sorting="defaultEventSorting" :table-grouping-options="tableGroupingOptions" sticky="header" :virtualize="{ estimateSize: 65, overscan: 8 }" empty="暂无事件记录。" table-key="events" table-min-width="1180px" class="admin-table">
        ...
        <template #name-cell="{ row }"><div v-if="row.getIsGrouped()" class="flex items-center gap-2"><UButton class="hit-44" size="sm" color="neutral" variant="ghost" square :icon="row.getIsExpanded() ? 'i-lucide-chevron-down' : 'i-lucide-chevron-right'" :aria-label="row.getIsExpanded() ? '收起分组' : '展开分组'" @click="row.toggleExpanded()" /><strong>{{ groupLabel(row.groupingColumnId ?? "", row.getValue(row.groupingColumnId ?? "")) }}</strong><span class="text-sm text-muted">{{ row.subRows.length }} 条</span></div><strong v-else class="block truncate" :title="row.original.name">{{ row.original.name }}</strong></template>
        ...
        <template #actions-cell="{ row }"><div v-if="!row.getIsGrouped()" class="table-actions"><UButton label="编辑" size="sm" color="neutral" variant="outline" @click="openEvent(row.original)" /></div></template>
      </AdminDataTable>
    </section>
