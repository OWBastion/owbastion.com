...
const claims = ref<Claim[]>([]); const invitations = ref<Invitation[]>([]); const loading = ref(true); const errorMessage = ref(""); const actionMessage = ref("");
...
async function revokeInvitation() { if (!revokeTarget.value || !revokeReason.value.trim()) return; revoking.value = true; actionMessage.value = "保存中…"; try { await api(`/v1/binding-invites/${revokeTarget.value.inviteId}/revoke`, { method: "POST", headers: { "Idempotency-Key": crypto.randomUUID() }, body: { contractVersion: "1", reason: revokeReason.value.trim() } }); revokeTarget.value = null; revokeReason.value = ""; actionMessage.value = "邀请码已撤销"; await load(); } catch { actionMessage.value = "无法撤销邀请码"; } finally { revoking.value = false; } }
onMounted(load);
</script>
