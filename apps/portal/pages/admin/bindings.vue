...
async function decide(claim: Claim, decision: "approved" | "rejected") { const action = decision === "approved" ? "批准" : "拒绝"; try { await api(`/v1/binding-claims/${claim.claimId}/decision`, { method: "POST", headers: { "Idempotency-Key": crypto.randomUUID() }, body: { contractVersion: "1", decision } }); toast.add({ title: `申请已${action}`, color: "success" }); await load(); } catch { toast.add({ title: "无法处理申请", color: "error" }); } }
...
async function revokeInvitation() { if (!revokeTarget.value) return; revoking.value = true; try { const reason = revokeReason.value.trim(); await api(`/v1/binding-invites/${revokeTarget.value.inviteId}/revoke`, { method: "POST", headers: { "Idempotency-Key": crypto.randomUUID() }, body: { contractVersion: "1", ...(reason ? { reason } : {}) } }); revokeTarget.value = null; revokeReason.value = ""; toast.add({ title: "邀请码已撤销", color: "success" }); await load(); } catch { toast.add({ title: "无法撤销邀请码", color: "error" }); } finally { revoking.value = false; } }
