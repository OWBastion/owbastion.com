...
const invitationColumns = [{ accessorKey: "battleTag", header: "玩家" }, { accessorKey: "status", header: "状态" }, { accessorKey: "expiresAt", header: "有效期" }];
...
<template>
  <AdminWorkspace title="绑定管理" :count="loading ? '读取中…' : `${claims.length} 条`">
    ...
    <section class="binding-section" aria-labelledby="invites-title">
...
<style scoped>.binding-section { display:grid; gap:12px; }.claim-actions { display:flex; gap:8px; }.table-meta { color:var(--quiet); font-size:.78rem; }</style>
