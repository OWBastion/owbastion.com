...
<template>
  <form class="review-editor" @submit.prevent="emit('save')">
    ...
    <label class="review-comment-field"><span>评价内容 <span class="review-optional">（可选）</span></span><textarea :value="comment" maxlength="500" rows="4" :disabled="saving" aria-describedby="review-anonymous-note" placeholder="分享你的实际体验" @input="emit('update:comment', ($event.target as HTMLTextAreaElement).value)"></textarea></label>
