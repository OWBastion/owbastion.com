...
<template>
  <section class="catalog-section" aria-labelledby="achievement-catalog-title">
    ...
    <section v-if="mapChallenges.length" class="achievement-section"><div class="group-heading"><div><h3>选择地图后查看可用目标</h3></div></div><USelect v-model="selectedMapId" aria-label="选择地图" placeholder="选择地图" :items="mapItems" /></section>
    ...
    <section v-if="scheduledChallenges.length" class="achievement-section upcoming-section" aria-labelledby="upcoming-achievements-title">
      <div class="group-heading"><div><h3 id="upcoming-achievements-title">未开放</h3></div><span>{{ scheduledChallenges.length }} 个称号</span></div>
      <div class="achievement-grid"><article v-for="challenge in scheduledChallenges" :key="challenge.challengeId" class="achievement-card upcoming"><strong>{{ challenge.titleName }}</strong><span v-if="challenge.scope === 'map'" class="card-kicker">{{ mapVariantLabel(challenge.mapVariant) }}</span><span>{{ challenge.condition }}</span><small>暂不接受截图提交。</small></article></div>
    </section>
    <p v-if="!automaticChallenges.length && !manualGroups.length && !scheduledChallenges.length" class="empty-state">暂无可提交的成就挑战。地图通关仍可提交。</p>
  </section></template>

<style scoped>
.catalog-section, .achievement-section, .automatic-section { display: grid; gap: 16px; }.catalog-section { padding: 2px; }.catalog-heading { display: grid; }.group-heading p { margin: 0; }.catalog-heading h2 { margin: 0; color: var(--text); font-size: 1.35rem; letter-spacing: -.04em; }
