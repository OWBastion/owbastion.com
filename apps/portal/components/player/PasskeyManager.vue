...
async function load() {
  ...
  error.value = "";
  try {
    const response = await api<{ contractVersion: "1"; items: Passkey[] }>("/v1/me/passkeys");
...
async function remove() {
  if (!pendingRemoval.value || items.value.length < 2) return;
  saving.value = true;
  error.value = "";
  try {
...
<template>
  <section class="passkey-manager surface-card" aria-labelledby="passkeys-title">
    <div class="passkey-heading">
      <div>
        <h2 id="passkeys-title" class="section-title">Passkey</h2>
        <p class="passkey-note">Passkey 用于登录 Portal。请保留至少一个可用的 Passkey。</p>
      </div>
    ...
    <UEmpty v-else-if="items.length === 0" title="尚无 Passkey" description="添加 Passkey 后，可以使用设备凭据登录。" variant="naked" />
    <ul v-else class="passkey-list">
      <li v-for="item in items" :key="item.passkeyId" class="passkey-row">
        ...
        <UButton label="移除" color="error" variant="soft" size="sm" :disabled="items.length < 2 || saving || passkeyBusy" @click="pendingRemoval = item" />
      </li>
    </ul>
