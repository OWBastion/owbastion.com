...
const openSelectedMap = () => {
  ...
  if (selectedMap.value?.mapId !== map.mapId || !modalOpen.value) showMap(map);
};
...
.map-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(min(100%, 17rem), 1fr)); gap: var(--space-4); }
</style>
