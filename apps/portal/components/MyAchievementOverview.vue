...
<template>
  <section class="achievement-summary surface-card" aria-label="成就概览">
    <article class="summary-metric"><UIcon name="i-lucide-trophy" aria-hidden="true" /><strong>{{ earnedCatalogCount }}<small> / {{ challenges.length }}</small></strong><span>已获得</span></article>
    <article class="summary-metric"><UIcon name="i-lucide-chart-no-axes-combined" aria-hidden="true" /><strong>{{ completionRate }}<small>%</small></strong><span>完成率</span></article>
    <article class="summary-metric summary-metric-latest"><UIcon name="i-lucide-sparkles" aria-hidden="true" /><div><span>最近获得</span><strong>{{ recentTitles[0]?.label ?? "暂无称号" }}</strong></div></article>
  </section>
  ...
  <div class="achievement-layout">
      ...
      <section v-for="group in groups" :key="group.category" class="achievement-section" :aria-labelledby="`my-category-${group.category}`">
        ...
        <div class="achievement-grid">
          <article v-for="challenge in group.challenges" :key="challenge.challengeId" class="achievement-card" :class="{ earned: ownedTitleKeys.has(challenge.titleKey) }">
            ...
            <div class="achievement-copy"><strong>{{ challenge.titleName }}</strong><span>{{ challenge.condition }}</span><span v-if="ownedTitleKeys.has(challenge.titleKey)" class="earned-status">已获得</span><span v-else-if="challenge.status === 'scheduled'" class="status">未开放</span><span v-else-if="challenge.status === 'sunsetting'" class="status">即将结束</span></div>
          </article>
        </div>
      </section>
      ...
      <section v-if="historicalTitles.length" class="achievement-section historical-section" aria-labelledby="historical-title">
        ...
        <div class="achievement-grid">
          <article v-for="title in historicalTitles" :key="title.grantId" class="achievement-card earned">
            ...
            <div class="achievement-copy"><strong>{{ title.label }}</strong><span>{{ title.condition }}</span><span class="earned-status">已获得</span></div>
          </article>
        </div>
      </section>
    ...
    <aside class="achievement-sidebar" aria-label="成就统计">
      <section class="sidebar-card surface-card" aria-labelledby="recent-title"><header class="sidebar-heading"><h2 id="recent-title">最近获得</h2></header><div v-if="recentTitles.length" class="recent-list"><article v-for="title in recentTitles" :key="title.grantId" class="recent-item"><div class="recent-icon" aria-hidden="true"><img v-if="title.iconUrl" :src="title.iconUrl" alt="" /><UIcon v-else :name="`i-lucide-${title.icon}`" /></div><div><strong>{{ title.label }}</strong><span>{{ formatDate(title.grantedAt) }}</span></div></article></div><UEmpty v-else title="暂无称号" variant="naked" /></section>
