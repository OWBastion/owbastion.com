...
const parsedPlayer = computed(() => {
  const name = playerName.value.trim();
  const id = playerId.value.trim();
  return { name, id, valid: Boolean(name) && /^\d{1,10}$/.test(id) };
});
const canSubmit = computed(() => parsedPlayer.value.valid && !submitting.value);
...
async function createInvitation() {
  ...
  try {
    const response = await api<Invitation>("/v1/binding-invites", {
      ...
      body: { contractVersion: "1", playerName: parsedPlayer.value.name, playerId: parsedPlayer.value.id, historicalTitleGrantIds: selectedGrantIds.value },
    });
...
<template>
  <section class="invite-panel surface-card" aria-labelledby="invite-panel-title">
    ...
    <form class="invite-panel__form" @submit.prevent="createInvitation">
      <div class="invite-panel__identity">
        <UInput v-model="playerName" label="目标 BattleTag 名称" placeholder="玩家名称" aria-label="目标 BattleTag 名称" />
        <UInput v-model="playerId" label="数字 ID" inputmode="numeric" placeholder="1234" aria-label="目标 BattleTag 数字 ID" />
      </div>
