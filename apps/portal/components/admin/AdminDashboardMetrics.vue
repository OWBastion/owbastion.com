...
<style scoped>
.dashboard-metrics { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); overflow:hidden; }.metric { position:relative; display:grid; align-content:start; min-height:132px; padding:22px; border-right:1px solid var(--line); color:inherit; text-decoration:none;  }.metric:last-child { border-right:0; }.metric::before { content:""; width:7px; height:7px; margin-bottom:17px; border-radius:50%; background:var(--quiet); }.metric--accent::before { background:var(--accent); }.metric-label,.metric-detail { margin:0; color:var(--quiet); font-size:.72rem; font-weight:650; letter-spacing:.02em; }.metric-value { margin-top:5px; color:var(--text); font-size:clamp(1.8rem,3vw,2.45rem); font-weight:690; line-height:1; }.metric-detail { margin-top:10px; font-weight:500; letter-spacing:0; }.metric:hover,.metric:focus-visible { background:color-mix(in oklch,var(--surface-raised) 70%,transparent); }.metric:focus-visible { outline:2px solid var(--accent); outline-offset:-2px; }
