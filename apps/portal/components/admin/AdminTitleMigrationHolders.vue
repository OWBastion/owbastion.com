...
.holder-list { display: grid; flex: 1; min-height: 0; max-height: 560px; overflow-y: auto; overscroll-behavior: contain; scrollbar-gutter: stable; }.holder-item { display: flex; align-items: center; gap: 11px; min-width: 0; padding: 14px 18px; border: 0; border-left: 3px solid transparent; color: var(--text); background: transparent; text-align: left; transition: background 160ms ease, transform 100ms ease, border-color 160ms ease; }.holder-item + .holder-item { border-top: 1px solid var(--line); }.holder-item:hover { background: color-mix(in oklch, var(--surface-raised) 64%, transparent); }.holder-item:focus-visible { z-index: 1; outline: 3px solid var(--accent); outline-offset: -3px; }.holder-item:active { transform: scale(.99); }.holder-item--selected { border-left-color: var(--accent); background: var(--accent-surface); }.holder-avatar { display: grid; flex: 0 0 auto; width: 34px; height: 34px; place-items: center; border-radius: 50%; color: var(--on-accent); background: var(--accent); font-size: .82rem; font-weight: 750; }.holder-copy { display: grid; flex: 1; min-width: 0; gap: 2px; }.holder-copy strong, .holder-copy small { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.holder-copy strong { font-size: .86rem; }.holder-copy small { color: var(--quiet); font-size: .72rem; }.holder-state { padding: 28px 18px; color: var(--quiet); text-align: center; }.holder-pagination { display: flex; justify-content: center; }
