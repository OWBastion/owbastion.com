...
type Holder = { holderName: string; totalCount: number; unclaimedCount: number; status?: "pending" | "completed" };
  ...
  <UCard class="holder-panel" variant="outline">
    <template #header>
      <div class="panel-heading">
        ...
        <UBadge :label="`${total} 位`" color="neutral" variant="subtle" />
      </div>
      <div class="holder-search" role="search" aria-label="搜索称号记录">
    ...
    <UEmpty v-else-if="!holders.length" title="暂无匹配持有者" variant="naked" />
    <div v-else class="holder-list" role="listbox" aria-label="历史持有人">
      <button
        v-for="holder in holders"
        :key="holder.holderName"
        class="holder-item"
        :class="{ 'holder-item--selected': holder.holderName === selectedHolderName }"
        type="button"
        role="option"
        :aria-selected="holder.holderName === selectedHolderName"
    ...
    <template #footer>
      <div v-if="holderPages > 1" class="holder-pagination">
        <UPagination :page="page" :total="total" :items-per-page="pageSize" :disabled="loading" @update:page="emit('update:page', $event)" />
      </div>
    </template>
  </UCard>
</template>
...
.panel-heading h2 { margin: 0; font-size: 1.05rem; letter-spacing: -.025em; }
...
.holder-item { display: flex; align-items: center; gap: 11px; min-width: 0; padding: 14px 18px; border: 0; border-left: 3px solid transparent; color: var(--text); background: transparent; text-align: left; transition: background 160ms ease, transform 100ms ease, border-color 160ms ease; }
...
.holder-item:active { transform: scale(.99); }
.holder-item--selected { border-left-color: var(--accent); background: var(--accent-surface); }
