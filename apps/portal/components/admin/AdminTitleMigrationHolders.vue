...
const emit = defineEmits<{ select: [holder: Holder]; "update:filter": [filter: Filter]; "update:page": [page: number]; "update:query": [query: string]; search: [] }>();
...
<template>
  <UCard class="holder-panel" variant="outline">
    <template #header><div class="panel-heading"><div><p class="eyebrow">历史持有者</p><h2>迁移对象</h2></div><UBadge :label="`${total} 位`" color="neutral" variant="subtle" /></div><form class="holder-search" role="search" aria-label="搜索称号记录" @submit.prevent="emit('search')"><UInput :model-value="query" icon="i-lucide-search" placeholder="搜索持有者或称号" aria-label="搜索历史称号" @update:model-value="emit('update:query', $event)" /><UButton type="submit" label="搜索" color="neutral" variant="outline" :loading="loading" /></form></template>
...
.holder-search { display: flex; gap: 8px; margin-top: 15px; }.holder-search :deep(.portal-control) { flex: 1; min-width: 0; }
