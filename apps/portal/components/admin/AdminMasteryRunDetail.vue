...
<template>
  <div class="mastery-run-detail" :aria-busy="actionLoading || undefined">
    <section class="mastery-run-detail__summary" aria-labelledby="mastery-run-summary-title">
      ...
      <dl class="detail-grid mastery-run-detail__facts">
        <div class="detail-grid__row"><dt>玩家</dt><dd><NuxtLink :to="`/admin/players/${encodeURIComponent(detail.run.playerAccountId)}`">{{ detail.run.playerName }}</NuxtLink><span class="mastery-run-detail__quiet">{{ detail.run.playerId }}</span></dd></div>
        <div class="detail-grid__row"><dt>地图</dt><dd>{{ detail.run.mapName }}<span class="mastery-run-detail__quiet">{{ detail.run.mapId }}</span></dd></div>
        <div class="detail-grid__row"><dt>难度</dt><dd>{{ detail.run.difficulty }}</dd></div>
        <div class="detail-grid__row"><dt>通关码</dt><dd class="mastery-run-detail__code">{{ detail.run.runCode }}</dd></div>
      ...
      <dl v-if="detail.run.status === 'invalidated'" class="detail-grid mastery-run-detail__invalidation">
    ...
    <section class="mastery-run-detail__section" aria-labelledby="mastery-run-source-title">
      ...
      <dl class="detail-grid mastery-run-detail__facts">
        <div class="detail-grid__row"><dt>提交编号</dt><dd class="mastery-run-detail__code">{{ detail.sourceSubmission.submissionId }}</dd></div>
    ...
    <section class="mastery-run-detail__section" aria-labelledby="mastery-run-xp-title">
      ...
      <div class="mastery-run-detail__split">
        <dl class="detail-grid mastery-run-detail__facts">
          ...
          <div class="detail-grid__row"><dt>挑战加成</dt><dd>{{ detail.run.xpInputSnapshot.challengeBonus }}</dd></div>
      ...
      <dl v-if="eventCounters.length" class="detail-grid mastery-run-detail__event-counters">
    ...
    <section class="mastery-run-detail__section" aria-labelledby="mastery-run-conflicts-title">
      ...
      <div v-else class="mastery-run-detail__conflicts">
        <article v-for="conflict in detail.conflicts" :key="conflict.submissionId" class="mastery-run-detail__conflict">
          ...
          <dl v-if="conflict.resolution" class="detail-grid mastery-run-detail__resolution">
