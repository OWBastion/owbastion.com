...
<template>
  <UCard v-if="holder" class="detail-panel" variant="outline">
    ...
    <AdminDataTable v-model:sorting="grantSorting" :sorting-options="grantSortingOptions" :default-sorting="defaultGrantSorting" :data="holder.grants" :columns="columns" :mobile-columns="[{ id: 'title', priority: 'primary', order: 0 }, { id: 'status', priority: 'primary', order: 1 }, { id: 'category', priority: 'detail', order: 2 }]" row-key="grantId" :loading="loading || saving" empty="暂无称号记录。" table-key="title-migration-grants" table-min-width="620px" class="admin-table">
      <template #actions-cell="{ row }"><div class="table-actions"><UButton v-if="row.original.status === 'unclaimed'" label="关联" color="neutral" variant="outline" size="sm" :disabled="!selectedPlayerId || saving" @click="emit('grant', row.original)" /><UButton v-else-if="row.original.status === 'active'" label="撤销" color="neutral" variant="link" size="sm" :disabled="saving" @click="emit('revoke', row.original)" /></div></template>
    </AdminDataTable>
  </UCard>
