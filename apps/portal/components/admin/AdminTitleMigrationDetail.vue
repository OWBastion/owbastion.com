...
<style scoped>
.detail-panel { container-type: inline-size; min-width: 0; }
.detail-panel :deep([data-slot="header"]) { padding: var(--space-5); }
...
.detail-action :deep(.form-field) { gap: var(--space-1); }
.player-state { margin: 0; width: 100%; color: var(--quiet); font-size: var(--type-caption-size); }
.player-state--error { color: var(--danger); }
.player-load-more { display: grid; gap: var(--space-2); width: 100%; }

.grant-pagination { display: flex; align-items: center; justify-content: space-between; gap: var(--space-3); padding: var(--space-3) var(--space-1) var(--space-1); }
.grant-pagination__meta { margin: 0; color: var(--quiet); font-size: var(--type-caption-size); }
.detail-panel--empty { display: grid; min-height: 360px; place-items: center; }
.detail-panel :deep(.title-cell) { display: grid; gap: var(--space-1); }
.detail-panel :deep(.title-cell small) { color: var(--quiet); font-size: var(--type-caption-size); }
