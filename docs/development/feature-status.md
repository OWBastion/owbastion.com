...
| Map title rule model | `coded` | Platform owns a reusable `map_title_rules` entity (one row per kind) with `map_title_rule_exceptions` for per-map overrides and `map_title_rule_compat` for legacy ID stability; `0049` migrates standard rules, `0050` restores the scoped CLASSIC challenge, and `0052` persists its OCR-required map variant; `map_title_rewards` remains read-only during the compatibility window | `migrations/0048_map_title_rules.sql`, `migrations/0049_migrate_standard_map_title_rules.sql`, `migrations/0050_migrate_classic_map_title.sql`, `migrations/0052_add_title_challenge_map_variant.sql`, `tools/map-title-rule-reconciliation.sql`, `packages/database/src/schema.ts` (mapTitleRules/mapTitleRuleExceptions/mapTitleRuleCompat), `packages/database/src/index.ts` (resolveMapTitleProjection/resolveCompatProjection, snapshot review path), `packages/database/src/map-title-rule.test.ts` | Archive pre/post production reconciliation and validate Bastion build-token projections before legacy cleanup |
...
| Public Agents content API | `coded` | Public read-only projection over published events, maps, achievements, titles, and public title-holding facts; ordinary requests omit numeric player IDs, while requests carrying the Bastion build token may receive current player display names, stable game player IDs, active title keys, and required map scope; no QQ, submission, review-source, time, or audit data | `/v1/agents/*`, Agent contracts/domain/database projections, API tests | Add deployed query and cache verification |
...
| Historical title migration and grants | `coded` | Platform owns historical title records and records historical and direct manual Grants | Generic Grant API, `/admin/titles`, `/admin/grants`, audit/idempotency code | Add production operational verification |
