...
| Map title rule model | `coded` | Platform owns a reusable `map_title_rules` entity (one row per kind) with `map_title_rule_exceptions` for per-map overrides and `map_title_rule_compat` for legacy ID stability; `map_title_rewards` is a read-only compatibility layer during the migration window | `migrations/0048_map_title_rules.sql`, `packages/database/src/schema.ts` (mapTitleRules/mapTitleRuleExceptions/mapTitleRuleCompat), `packages/database/src/index.ts` (resolveMapTitleProjection/resolveCompatProjection, snapshot review path), `packages/database/src/map-title-rule.test.ts` | Wire rule resolution into upload-session creation and compat seeding |
