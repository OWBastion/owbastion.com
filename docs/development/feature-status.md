...
| Versioned content release plane | `coded` | Platform owns Draft/Change Set/Candidate/Release state; existing admin catalog pages remain the single working-content editor, while release Portal captures and diffs that catalog automatically; maintainers choose Next or Release and never operate Change Set directly | `migrations/0034_release_plane.sql`, release contracts/domain service, `/v1/admin/releases/drafts/from-catalog`, `/v1/admin/releases/drafts/{draftId}/confirm`, Portal release diff page, `/v1/internal/bastion/*`, Bastion candidate workflow | Add deployed platform-to-Bastion callback evidence |
