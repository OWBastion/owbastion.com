...
- Use `page-shell` (broad default workspaces), `page-shell--narrow`
  (forms / focused detail), `page-shell--wide` (admin data workspaces).
  Approximate max widths: `1440px`, `760px`, `1440px` — these baselines live
  in `main.css`.
- Default to `page-shell` for card/grid directories, dashboards, editorial
  ...
  global header measure. Reserve `page-shell--narrow` for focused forms and
  prose detail. Do not compensate for the shared default with page-local
  widths.
