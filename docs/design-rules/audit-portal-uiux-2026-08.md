...
2. **玩家侧文案与术语失守**：禁用词「发放」、非规范状态词、「（可选）」标记（明令禁止）。
...
| # | 位置 | 问题 | 规则引用 | 修复 |
| --- | --- | --- | --- | --- |
| S-01 | `components/MyAchievementOverview.vue:69`（渲染于 `pages/achievements.vue`） | 玩家侧 badge「不再发放」使用禁用词 | terminology.md §玩家侧禁用词：发放 → 获得 | 改「不再获得」或「已停止获得」 |
