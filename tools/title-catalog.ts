...
const titleColor = (title: TitleCatalogSnapshot["titles"][number]) => {
  const expression = title.colorExpr ?? "null";
  const hero = expression.match(/^heroColor\[(\d+)\]$/);
  if (hero) return JSON.stringify({ kind: "heroColor", index: Number(hero[1]) });
  const rgb = expression.match(/^vect\((\d+), (\d+), (\d+)\)$/);
  if (rgb) return JSON.stringify({ kind: "rgb", value: rgb.slice(1).map(Number) });
  const palette = expression.match(/^breathPalette\.(orange|red|purple|gold|blue)$/);
  if (palette) return JSON.stringify({ kind: "palette", name: palette[1] });
  if (expression === "null") return "null";
  throw new Error(`Unsupported title color expression: ${expression}`);
};
